...
function SetupPageInner() {
  ...
  const handleAutoPublish = async (topicList: TopicOption[], count: number) => {
    ...
    const runOneTopic = async (topic: TopicOption, blogNum: number) => {
      ...
      // 4. CTA
      stepLabel(4);
      const ctaRes = await fetch("/api/cta-agent", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ optimizedContent: optimized, businessContext: context, topicTitle: topic.title }) });
      ...
      stepLabel(6);
      const publishRes = await fetch("/api/publish-agent", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ optimizedContent: optimized, meta, schema, cta, images, businessContext: context, templateId: "test-template", saveAsDraft: true }) });
